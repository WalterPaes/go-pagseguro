...
var (
	...
	BadRequestResponse = "{\n\t\"error_messages\": [{\n\t\t\t\"code\": \"1001\",\n\t\t\t\"description\": \"required_parameter\",\n\t\t\t\"parameter_name\": \"payment_method.card.number\",\n\t\t\t\"info\": \"https://api.pagseguro.com.br/errors/invalid_request_body\"\n\t\t},\n\t\t{\n\t\t\t\"code\": \"1002\",\n\t\t\t\"description\": \"invalid_parameter\",\n\t\t\t\"parameter_name\": \"payment_method.card.address.country\",\n\t\t\t\"info\": \"https://api.pagseguro.com.br/errors/invalid_request_body\"\n\t\t}\n\t]\n}"
)

type HttpRequesterMockSuccess struct {
	Headers map[string]string
}

func (r *HttpRequesterMockSuccess) SetHeaders(headers map[string]string) net.Requester {
	r.Headers = headers
	return r
}

func (_ HttpRequesterMockSuccess) DoRequest(r *http.Request) (*http.Response, error) {
	var body string
...
func (_ HttpRequesterMockSuccess) ReadBody(body io.Reader) ([]byte, error) {
...
type HttpRequesterMockFail struct {
	Headers map[string]string
}

func (r *HttpRequesterMockFail) SetHeaders(headers map[string]string) net.Requester {
	r.Headers = headers
	return r
}

func (_ HttpRequesterMockFail) DoRequest(r *http.Request) (*http.Response, error) {
	response := &http.Response{
		StatusCode: http.StatusBadRequest,
		Body:       ioutil.NopCloser(bytes.NewBufferString(BadRequestResponse)),
	}
	return response, nil
}

func (_ HttpRequesterMockFail) ReadBody(body io.Reader) ([]byte, error) {
	result, err := ioutil.ReadAll(body)
	if err != nil {
		return nil, err
	}
